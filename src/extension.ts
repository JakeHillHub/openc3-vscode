...
async function preFlightChecks(excludePattern: string): Promise<boolean> {
  ...
  /* Use a narrow set of files that would likely not exist outside of a cosmos project */
...
const GITIGNORE_PREFIX = '\n# Auto-generated by MyCosmosExtension\n';

async function updateGitignore(targetModuleDirs: string[]) {
  ...
  try {
    if (!fs.existsSync(gitignoreUri.fsPath)) {
      fs.writeFileSync(gitignoreUri.fsPath, '');
      return;
    }
    ...
    const ignoreLinesOut = [];
    for (const targetModuleDir of targetModuleDirs) {
      ignoreLinesOut.push(`${targetModuleDir}/**/*.pyi`);
    }

    if (!content.includes('**/*.pyi')) {
      const newContent = new TextEncoder().encode(content + GITIGNORE_PATTERN);
      await vscode.workspace.fs.writeFile(gitignoreUri, newContent);
      console.log('.gitignore updated to exclude .pyi files.');
    }
  } catch (error) {
    ...
    if (error instanceof vscode.FileSystemError && error.code === 'FileNotFound') {
      await vscode.workspace.fs.writeFile(
        gitignoreUri,
        new TextEncoder().encode(GITIGNORE_PATTERN.trim())
      );
      console.log('.gitignore created and configured to exclude .pyi files.');
    }
  }
}
